...
//
// IMPORTANT: expo-notifications is required LAZILY inside try/catch. OTA
// updates share one JS bundle across binaries (runtimeVersion policy =
// appVersion, pinned 1.0.0), and binaries built BEFORE the module was added
// don't contain its native code — a top-level import would crash them on the
// next OTA update. Old binaries just no-op here.
...
function loadNotifications(): any | null {
  ...
  try {
    ...
    return null; // binary predates expo-notifications — silently unsupported
  }
}
