...
- **Zoom AI Companion summaries are the Phase 4 source, not recordings.** 29 exist on the stake account (Mar–Aug 2026) even though cloud recordings are empty — don't conclude "no Zoom data" from the recordings list. List: `GET zoom.us/rest/meeting/host_summary_list?page=N&hostId=` (cookie auth; page until `hasNextPage` is false — `total` returns 0). The summary BODY is in a cross-origin Zoom Docs iframe and cannot be scraped; content needs the real Zoom API, which needs the MCP re-authorized (all three Zoom MCP servers currently 401). Each summary is Attendees / Quick recap / Next steps grouped by person — **extract only "Next steps"; the recap carries pastoral substance and must never be stored or sent to a model.**
